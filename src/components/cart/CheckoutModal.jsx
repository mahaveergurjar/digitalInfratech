      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 sm:p-8 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row gap-4">
          ...
          <button
            ...
            form="checkout-form"
            disabled={submitting}
            className="w-full flex-1 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 disabled:cursor-not-allowed text-white font-bold py-4 px-8 rounded-xl shadow-lg hover:shadow-xl transition-all text-lg flex justify-center items-center gap-2"
          >
            <span>{submitting ? 'Placing order...' : 'Place Order'}</span>
            {!submitting && <span className="bg-white/20 px-2 py-0.5 rounded text-sm">{money.format(total)}</span>}
          </button>
        </div>
      </div>
    </div>
