...
export default function Services() {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* ─── HERO ─── */}
      <section
        className="relative overflow-hidden rounded-3xl text-white p-8 sm:p-12 lg:p-16"
        style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 60%, #0f172a 100%)' }}
      >
        <div className="hero-orb w-[450px] h-[450px] bg-orange-500/15 top-[-100px] right-[-80px]" />
        <div className="hero-orb w-[300px] h-[300px] bg-blue-500/10 bottom-[-60px] left-[-50px]" />
        <div className="hero-orb w-[180px] h-[180px] bg-violet-400/10 top-[40%] right-[30%]" />

        <div className="relative z-10 max-w-2xl">
          <div className="inline-flex items-center gap-2.5 bg-white/10 backdrop-blur border border-white/20 rounded-full px-4 py-2 text-xs font-bold uppercase tracking-wider mb-6">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black leading-[1.1] mb-4">
            Book trusted home{' '}
            <span className="text-shimmer">service experts</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-2.5">
            {serviceCategories.map((cat) => (
              <button
                ...
                className="inline-flex items-center gap-1.5 bg-white/10 hover:bg-white/20 border border-white/15 hover:border-white/30 rounded-xl px-3 py-1.5 text-sm font-semibold transition-all"
              >
                {cat.icon} {cat.label}
              </button>
            ))}
